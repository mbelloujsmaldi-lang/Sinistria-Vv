...
import { chargerRegistre, registreVersCsv, type FiltresRegistre } from "@/lib/registre";

// GET /api/registre/export — export CSV du Registre filtré (Sprint 25),
// mêmes filtres et même garde que /vv (tout profil actif). Réutilise
// exactement la même requête (lib/registre.ts, même LIMITE_REGISTRE) que
// la page : le CSV contient toujours exactement les lignes affichées,
// jamais le registre entier.
export async function GET(request: NextRequest) {
  ...
  const { lignes } = await chargerRegistre(supabase, filtres);
