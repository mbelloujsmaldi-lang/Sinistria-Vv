...
// capricieux ; un PNG 1080x320 (≈ 550 dpi pour 50 mm) est fiable et net à l'impression.
...
export async function rasteriserLogo(): Promise<string | null> {
  try {
    ...
    const svg = (await res.text()).replace("<svg ", '<svg width="1080" height="320"' + " ");
    ...
    try {
      ...
      if (!ctx) return null;
      ctx.drawImage(img, 0, 0, 1080, 320);
